...
c Write encounter details to appropriate files
          if (isbinary) then
c RAS--Changed to hack star1
c             WRITE(*,*) "binary",iclo,jclo,id(iclo),id(jclo)
            if (timestyle.eq.1) then
              if (unit(iclo).ge.10) then
                if (jclo.eq.0) then
                 write (unit(iclo),fout) year,month,
     %           t1,'STAR1',dclo,a1,e1,i1,a2,e2,i2
                else 
                 write (unit(iclo),fout) year,month,
     ...
     %           t1,'STAR1',dclo,a2,e2,i2,a1,e1,i1
                else 
                 write (unit(jclo),fout) year,month,
